    ...
    while (iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet)):
        ...
        if entireSet:
            for i in range(oS.m):
                ...
                print('fullSet, iter: %d i: %d, pairs changed %d' % (iter,
                                                    i, alphaPairsChanged))
            iter += 1
        else:
            ...
            for i in nonBoundIs:
                alphaPairsChanged += innerL(i, oS)
                print('non-bound, iter: %d i: %d, pairs changed %d' %(iter,
                                                    i, alphaPairsChanged))
            iter += 1
        if entireSet:
            entireSet = False
...
def testRbf(k1 = 1.0):
    ...
    errorCount = 0
    for i in range(m):
        kernelTransEval = kernelTrans(sVs, dataMat[i, :], ('rbf', k1))
        predict = kernelTransEval.T * multiply(labelSV, alphas[svInd]) + b
